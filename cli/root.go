// Package cli assembles the lichess command tree from the lichess
...
// NewApp assembles the kit application from the lichess domain. The
...
//
// To add a command, declare it in lichess/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := lichess.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(lichess.Domain{}).Register(app)
