...
describes what users get. Pick a format with `-o`, or let lichess choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
lichess <command> -o table   # aligned columns for reading
lichess <command> -o jsonl   # one JSON object per line, for piping
lichess <command> -o json    # a single JSON array
lichess <command> -o csv     # spreadsheet friendly
lichess <command> -o tsv     # tab-separated
lichess <command> -o url     # just the URL column
lichess <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
lichess <command> --fields id,title,url
```
...
```bash
lichess <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
lichess <command>            # a table, because this is a terminal
lichess <command> | wc -l    # JSONL, because this is a pipe
```
